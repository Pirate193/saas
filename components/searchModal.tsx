...
export const SearchModal = ({ open, onOpenChange }: SearchModalProps) => {
  ...
  const filteredFolders = folders?.filter((folder) =>
    folder.name.toLowerCase().includes(search.toLowerCase().trim())
  );
  const filteredNotes = notes?.filter((note) =>
    note.title.toLowerCase().includes(search.toLowerCase().trim())
  );
  const filteredFiles = files?.filter((file) =>
    file.fileName.toLowerCase().includes(search.toLowerCase().trim())
  );
          ...
          <div className="space-y-2">
            ...
            {filteredFolders?.map((folder) => (
              <div
                ...
                className="flex items-center gap-2 hover:bg-primary/50 p-2 rounded-2xl cursor-pointer"
              >
            ...
            {filteredFiles?.map((file) => (
              <div
                key={file._id}
                onClick={() => router.push(`/files/${file._id}`)}
                className="flex items-center gap-2 hover:bg-primary/50 p-2 rounded-2xl cursor-pointer"
              >
